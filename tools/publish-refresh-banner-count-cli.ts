...
// yet is a no-op, not a failure — not every staged snapshot carries one. The actual work lives in
// `refreshBannerCount` (`llms-version-banner.ts`), so `settle-since-markers.ts` can call the same
// regeneration step in-process instead of shelling out to this file a second time.
...
const count = refreshBannerCount(root);
if (count !== undefined) {
  console.log(count);
}
