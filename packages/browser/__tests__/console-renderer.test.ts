...
describe("bounded call-tree walk (cyclic and very deep trees)", () => {
  ...
  // chain one link longer than the limit exercises the identical code path — measured ~72ms run
  ...
  it("does not stack-overflow on a chain just past the depth limit", () => {
    vi.spyOn(console, "log").mockImplementation(() => {});
    vi.spyOn(console, "group").mockImplementation(() => {});
    vi.spyOn(console, "groupEnd").mockImplementation(() => {});

    expect(() => renderToConsole(traceTree([deepChain(10_001)]))).not.toThrow();
    vi.restoreAllMocks();
  }, 3_000);
});
