...
describe("createWinstonEventConsumer", () => {
  test("enter event logs class and method", () => {
    ...
    expect(entries[0]?.message).toBe("→ OrderService.placeOrder");
  ...
  test("exit with return logs outcome and value", () => {
    ...
    expect(entries).toHaveLength(1);
    expect(entries[0]?.message).toBe('← returned: "OK"');
  ...
  test("exit with exception logs the constructor name and warns by default", () => {
    ...
    expect(entries).toHaveLength(1);
    expect(entries[0]?.message).toBe("!! TypeError: boom");
  ...
  test("exception with error context appends and sanitizes [context]", () => {
    ...
    expect(entries[0]?.message).toBe("!! Error: bad\\nline [no such order 7\\ninjected]");
  ...
  test("exit with non-Error thrown value uses its typeof as the type", () => {
    ...
    expect(entries[0]?.message).toBe("!! string: string-error");
  });
  ...
  test("carries nt.runName on both enter and exit when a runName is given", () => {
    const { logger, entries } = createTestLogger();
    const consumer = createWinstonEventConsumer(logger, { runName: "bold elk soars" });
  ...
  test("exit with incomplete outcome logs a warning", () => {
    ...
    expect(entries).toHaveLength(1);
    expect(entries[0]?.message).toBe("← incomplete");
