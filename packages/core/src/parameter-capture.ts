...
export interface ParameterCapture {
   ...
   * several visible ones has not had its whole value withheld. See `renderCapture` in
   * `value-renderer.ts`, the seam both capture paths use to learn the top-level decision without
   * inferring it from the rendered string.
   */
