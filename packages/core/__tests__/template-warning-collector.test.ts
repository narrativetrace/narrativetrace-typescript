...
describe("bounded tree walk (cyclic and very deep trees)", () => {
  ...
  // chain one link longer than the limit exercises the identical code path — measured ~64ms run
  ...
  test("does not stack-overflow on a chain just past the depth limit", () => {
    expect(() => collectTemplateWarnings(traceTree([deepChain(10_001)]))).not.toThrow();
  }, 3_000);
});
