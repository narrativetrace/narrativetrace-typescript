...
describe("createHonoNarrativeContext", () => {
  test("the seam's default is 8192, argued in its own doc comment", () => {
    expect(DEFAULT_HONO_BUFFER_CAPACITY).toBe(8192);
  });
  ...
  test("bufferCapacity override actually resizes the ring, not just accepted and ignored", () => {
    ...
    const small = createHonoNarrativeContext(new NarrativeTraceConfig("detail"), {
      bufferCapacity: 2,
    });
    traceCalls(small, 5);
    expect(small.traceLoss().droppedEvents).toBeGreaterThan(0);

    const large = createHonoNarrativeContext(new NarrativeTraceConfig("detail"), {
      bufferCapacity: 1000,
    });
    traceCalls(large, 5);
  ...
  test("an explicit pipeline overrides bufferCapacity — sizing becomes the caller's job", () => {
    ...
    const ctx = createHonoNarrativeContext(new NarrativeTraceConfig("detail"), {
      bufferCapacity: 2,
