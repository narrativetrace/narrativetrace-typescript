<!-- source: documentation/feature-guide.md blob 55a36402a669 | translated: 2026-09-16 | reviewed: - -->
...
Lo que **esta implementación** ofrece, desde la perspectiva de quien lo usa.
Este archivo es deliberadamente delgado: registra lo que entregan
los paquetes de TypeScript y
...
| Decoradores — `@traced` (nombres de parámetros), `@narrated` (narración con plantillas `{param}`), `@onError` (plantillas de contexto de error), `@notTraced` (ocultación) | Gratis | Ambos dialectos de decoradores (TC39 estándar y `experimentalDecorators` heredado), TS 5.0+; [guia-de-decoradores.md](guia-de-decoradores.md) |
| API programática para JS plano — configuración por método `traceObject(service, context, { methods: { method: { params, narration, onError, notTraced } } })` (gemelo de configuración de cada decorador) *(since 0.1.3)*, el atajo de mapa de nombres `{ method: ["paramA", …] }` y la API cruda de entrada/salida de `NarrativeContext` | Gratis | Conjunto completo de funcionalidades sin soporte de decoradores |
| Ocultación de datos sensibles — índices `@notTraced`, listas de campos `static notTraced`, lista de denegación `RedactionPolicy` por patrón de nombre, multilingüe y siempre activa (`password`, `token`, `contraseña`, `senha`, `motDePasse`, `密码`, …); los valores ocultados no pueden filtrarse a través de plantillas de narración/error | Gratis | |
| Cinco niveles de captura (`off` → `errors` → `summary` → `narrative` → `detail`), modificables en tiempo de ejecución mediante `config.level`; variable de entorno `NARRATIVETRACE_LEVEL` en Node | Gratis | `off` corta el circuito antes de cualquier trabajo de captura |
...
`arg1`, …. Por eso las anotaciones de tier 2 pesan de verdad:
`@traced("customerId", …)` o el mapa `paramNames` es la forma de
...
| `vitest` — fixture `narrativeTest`, archivos de traza por prueba (`md`/`mmd`/`puml`/`json`/`clarity-json`/`canonical-json`), resumen de consola + reporters de claridad, un subpath `/reporters` importable, artefacto estructural `.nt` libre de valores + trazas aprobadas, identidad por invocación con `.each` *(since 0.1.3)* | Gratis | |
...
| Artefacto estructural `.nt` — traza por escenario libre de valores, baseline de último-verde, pie de página de delta "Since last green", trazas aprobadas opcionales (`.approved.nt`/`.received.nt`, `pnpm run approve-narratives`) | Gratis | [structural-trace-format.md](../structural-trace-format.md) (todavía sin traducir) |
...
| Servidor MCP — transporte stdio real (`@modelcontextprotocol/sdk`), 7 herramientas de análisis, conecta directamente Claude Code / Cursor | En desarrollo (Pro) | Plan Enterprise, fase E5 |
...
| Conjunto de auditoría y cumplimiento | Planificada (Pro) | Con gate |
...
1. Cada funcionalidad visible para el usuario **de esta implementación** aparece
   aquí, exactamente una vez, con un estado. Este archivo nunca repite
...
3. Los cambios que añaden o promueven una funcionalidad en este
   repositorio deben actualizar este archivo en el mismo commit.
4. Cualquier comportamiento propio de esta implementación (nombres de
   nivel, nombres de parámetros en tiempo de ejecución, sin
   instrumentación estilo agente) se declara aquí directamente.
