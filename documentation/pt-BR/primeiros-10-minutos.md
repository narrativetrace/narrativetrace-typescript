...
Node — importar apenas `@narrativetrace/core` lança uma exceção na primeira chamada rastreada.
`pnpm init` grava um `package.json` com `"type": "module"`, então a sintaxe `import` abaixo
